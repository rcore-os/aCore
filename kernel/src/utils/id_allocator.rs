use alloc::boxed::Box;
use core::ops::Range;

use bitmap_allocator::{BitAlloc, BitAlloc256, BitAlloc4K, BitAlloc64K};

use crate::error::{AcoreError, AcoreResult};

trait IdAllocatorWrapper: Send + Sync {
    fn new(range: Range<usize>) -> Self
    where
        Self: Sized;
    fn alloc(&mut self) -> AcoreResult<usize>;
    fn dealloc(&mut self, id: usize);
}

pub struct IdAllocator(Box<dyn IdAllocatorWrapper>);

impl IdAllocator {
    pub fn new(range: Range<usize>) -> AcoreResult<Self> {
        Ok(match range.end {
            0..=0x100 => Self(Box::new(IdAllocator256::new(range))),
            0x101..=0x1000 => Self(Box::new(IdAllocator4K::new(range))),
            0x1001..=0x10000 => Self(Box::new(IdAllocator64K::new(range))),
            _ => {
                warn!("out of range in IdAllocator::new(): {:#x?}", range);
                return Err(AcoreError::OutOfRange);
            }
        })
    }

    pub fn alloc(&mut self) -> AcoreResult<usize> {
        self.0.alloc()
    }

    pub fn dealloc(&mut self, id: usize) {
        self.0.dealloc(id)
    }
}

macro_rules! define_allocator {
    ($name: ident, $inner: ty) => {
        struct $name($inner);

        impl IdAllocatorWrapper for $name {
            fn new(range: Range<usize>) -> Self {
                let mut inner = <$inner>::DEFAULT;
                inner.insert(range);
                Self(inner)
            }

            fn alloc(&mut self) -> AcoreResult<usize> {
                self.0.alloc().ok_or(AcoreError::NoResources)
            }

            fn dealloc(&mut self, id: usize) {
                self.0.dealloc(id)
            }
        }
    };
}

define_allocator!(IdAllocator256, BitAlloc256);
define_allocator!(IdAllocator4K, BitAlloc4K);
define_allocator!(IdAllocator64K, BitAlloc64K);
